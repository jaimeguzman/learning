...
/*class LZWEncoder(val initial: String) {
  ...
  */
  def initTable(str: String) : Unit = {
    val map = new mutable.LinkedHashMap[String, Int]
    // toList produces list of Char => cast to String str.toList.removeDuplicates.zipWithIndex.foreach(x => map += (x._1.toString -> x._2))  dmap
  }
