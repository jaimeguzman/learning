...
private[trie] class TrieNode(val char:   Option[Char] = None,
  ...
  */
  ...
  def getHeightTrie[U](f: String => U): Int = {
    println("getHeightTrie::::::");
    var trieHeigth = 1
    @tailrec def foreachHelper(nodes: TrieNode*): Unit = {
      if (nodes.size != 0) {
        //println("\tSZ"+nodes.size )
  ...
  val lz78 = {
    ...
    breakable {
      for (i <- 0 to listaValores.size - 1) {
        ...
        if (trie.contains(listaValores(i)) && trie.contains(listaValores(i+1))) {
          print("esta en el diccionario y X es: " + listaValores(i) + "\n")
          Thread sleep 1000



          for( j <- i to listaValores.size-1 ){
                if(trie.contains(listaValores(j))) print("_ "+ listaValores(j) );
          }
